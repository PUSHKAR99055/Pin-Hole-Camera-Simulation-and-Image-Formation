...
def superimpose_cube_on_background(cube_image, background_image):
    ...
    for i in range(result_image.shape[0]):
        for j in range(result_image.shape[1]):
            ...
            if (cube_image[i, j, 0] == 255 and cube_image[i, j, 1] == 0 and cube_image[i, j, 2] == 0):
                # Pixel belongs to the cube (blue color in the cube)
                result_image[i, j] = cube_image[i, j]
...
def main():
    ...
    for t in time_range:  # Generate a sequence of images as a function of time
        ...
        T1 = T0 + velocity * t + 0.5 * acc * t * t
        T2 = T1 + np.array([10, 0, 0])
        ...
        v1 = MapIndex(Map2Da(K, R, T1, V1), c0, r0, p)
        v2 = MapIndex(Map2Da(K, R, T1, V2), c0, r0, p)
        v3 = MapIndex(Map2Da(K, R, T1, V3), c0, r0, p)
        v4 = MapIndex(Map2Da(K, R, T1, V4), c0, r0, p)
        v5 = MapIndex(Map2Da(K, R, T1, V5), c0, r0, p)
        v6 = MapIndex(Map2Da(K, R, T1, V6), c0, r0, p)
        v7 = MapIndex(Map2Da(K, R, T1, V7), c0, r0, p)
        v8 = MapIndex(Map2Da(K, R, T1, V8), c0, r0, p)
        
        # Map 3D vertices to 2D and then to pixel space for the second camera
        u1 = MapIndex(Map2Da(K, R, T2, V1), c0, r0, p)
        u2 = MapIndex(Map2Da(K, R, T2, V2), c0, r0, p)
        u3 = MapIndex(Map2Da(K, R, T2, V3), c0, r0, p)
        u4 = MapIndex(Map2Da(K, R, T2, V4), c0, r0, p)
        u5 = MapIndex(Map2Da(K, R, T2, V5), c0, r0, p)
        u6 = MapIndex(Map2Da(K, R, T2, V6), c0, r0, p)
        u7 = MapIndex(Map2Da(K, R, T2, V7), c0, r0, p)
        u8 = MapIndex(Map2Da(K, R, T2, V8), c0, r0, p)
        ...
        color = (255,0,0)
        # Draw the edges of the cube for the second camera (in blue)
        A = drawLine(A, u1, u2, color, thickness)  # Blue for the second camera
        A = drawLine(A, u2, u3, color, thickness)
        A = drawLine(A, u3, u4, color, thickness)
        A = drawLine(A, u4, u1, color, thickness)
        
        A = drawLine(A, u5, u7, color, thickness)
        A = drawLine(A, u6, u7, color, thickness)
        A = drawLine(A, u6, u8, color, thickness)
        A = drawLine(A, u8, u5, color, thickness)
        
        A = drawLine(A, u1, u7, color, thickness)
        A = drawLine(A, u2, u6, color, thickness)
        A = drawLine(A, u5, u4, color, thickness)
        A = drawLine(A, u3, u8, color, thickness)
        ...
        for i in range(r):
            for j in range(c):
                ...
                #(ir, jr) = ?????????????????????????
                (jr1, ir1) = MapIndex(Map2Da(K, R, T1, p1), c0, r0, p)
                (jr2, ir2) = MapIndex(Map2Da(K, R, T2, p2), c0, r0, p)

                if ((ir1 >= 0) and (jr1 >= 0) and (ir1 < Rows) and (jr1 < Cols)):
                    tmapval = tmap[i, j, 2]
                    A[ir1 ,jr1] = [ 0, 0, tmapval] # gray here, but [0, 0, tmpval] for red color output
                if ((ir2 >= 0) and (jr2 >= 0) and (ir2 < Rows) and (jr2 < Cols)):
                    tmapval = tmap[i, j, 2]
                    A[ir2 ,jr2] = [tmap[i, j, 2], 0, 0] # gray here, but [0, 0, tmpval] for red color output
